...
CORPUS_ID = 'DailyDialog'
SOURCE_FILE_LIST = ['train.json', 'valid.json', 'test.json']
...
def preprocess_dialog(sample, split_id, conversation_id):
    ...
    if sample['topic'] == 'culture_and_educastion':
        sample['topic'] = 'culture_and_education'
    # Pre-process dialogue
    dialogue = [
        [split_id, CORPUS_ID, conversation_id, idx, *preprocess_turn(turn), sample['topic']]
        for idx, turn in enumerate(sample['dialogue'])
    ]
    return dialogue
...
def main(args):
    ...
    for source_file_name, split_id in zip(SOURCE_FILE_LIST, SPLITS):
        ...
        with open(os.path.join(args.source_dir_path, source_file_name)) as f:
            data_set = [json.loads(line) for line in f.readlines()]
